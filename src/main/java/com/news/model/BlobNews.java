...
public class BlobNews {
	public static void main(String[] args) {
		String URL = "jdbc:mysql://localhost:3306/room?useUnicode=yes&characterEncoding=utf8&useSSL=true&serverTimezone=Asia/Taipei";
		String USER = "root";
		String PASSWORD = "ppom21256212";
		
		
		try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
			// 存圖片到資料庫
			String sql = "UPDATE news set news_pic=? where news_no = ?";
			try (PreparedStatement ps = connection.prepareStatement(sql);
				InputStream in = Files.newInputStream(Path.of("Hinata.JPG"))) { 
				ps.setBinaryStream(1, in, in.available());
				ps.setInt(2, 6);
				int rowCount = ps.executeUpdate();
				System.out.println(rowCount + " row(s) 新增!!");
			}
